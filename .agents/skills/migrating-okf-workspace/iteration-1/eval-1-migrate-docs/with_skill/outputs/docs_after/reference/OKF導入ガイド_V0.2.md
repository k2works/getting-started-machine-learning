...
description: OKF v0.2 仕様（バンドル構造・フロントマター・来歴/信頼/ライフサイクル・Attested Computation・適合性）の日本語整理と、本プロジェクトへの段階的な導入手順。
tags: [reference, okf, knowledge-bundle, specification]
status: stable
generated: { by: claude-code/claude-fable-5, at: 2026-08-25T02:18:46Z }
sources:
  - id: okf-spec
...
このドキュメントは、Google Cloud Platform が公開している [Open Knowledge Format (OKF) SPEC.md](https://github.com/GoogleCloudPlatform/open-knowledge-format/blob/main/SPEC.md) **Version 0.2** を基に、仕様の内容を日本語で整理し、プロジェクトに導入するための手順をまとめたものです。[^okf-spec]
...
[^okf-spec]: Open Knowledge Format — SPEC.md (v0.2)
