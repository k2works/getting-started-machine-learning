...
fn main() -> ExitCode {
    ...
    let result = match args.get(1).map(String::as_str) {
        ...
        Some("chapter14") => chapter14::run(&mut out).map_err(|e| e.to_string()),
        Some("chapter15") => chapter15::run(&mut out).map_err(|e| e.to_string()),
        _ => {
            let _ = writeln!(
                io::stderr(),
                "使い方: cargo run --bin chapters -- (chapter01 | chapter02 | chapter03 | chapter07 | chapter08 | chapter09 | chapter10 | chapter11 | chapter12 | chapter13 | chapter14 | chapter15)"
            );
