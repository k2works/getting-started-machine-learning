...
type: Reference
title: リリースガイド
description: 品質ゲート→バージョンバンプ→CHANGELOG 生成→git commit + tag を一貫して実行するリリースワークフローと、バージョニング規則・リリースタスク一覧・実行手順。
tags: [reference, release, versioning, changelog]
status: stable
...
[Semantic Versioning](https://semver.org/)[^semver] に従います。
...
CHANGELOG は [Conventional Commits](https://www.conventionalcommits.org/)[^conventional-commits] の規約に基づいて自動生成されます。
