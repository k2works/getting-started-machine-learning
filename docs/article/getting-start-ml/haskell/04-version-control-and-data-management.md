...
**なお、この 80% という値も後で下げることになりました。** 上の数値はこの章を書いた時点（663 式）のもので、章が 15 個そろうと 8206 式になり、**データが無いときのカバレッジは 71% まで落ちます**。実データのテストが増えたぶん、手元と CI の差が広がったからです。原則（いちばん条件の悪い環境を基準にする）は正しかったのですが、**その環境の数字自体が動いた**ことになります。経緯は[第 6 章](06-task-runner-and-ci-cd.md)で扱います。
