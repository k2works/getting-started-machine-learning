...
`[[2,1],[1,3]]` という、特異でもなんでもない 2×2 の連立方程式です。**この環境の hmatrix は LAPACK を正しく呼べません。** LAPACK が「引数 1（行列の次数）の値が不正」と言っているので、hmatrix が 32 ビット整数で渡しているのに、環境の OpenBLAS が 64 ビット整数のインターフェース（ILP64）である、という食い違いだと読めます。

**注目すべきは、`linearSolve` が例外ではなく `Nothing` を返したことです。** 型の上では「解けなかった」と区別がつかないので、そのまま使えば「特徴量の列が独立でない」という **間違った診断** になります。C ライブラリとの境界では、`Maybe` で包まれていても中身の正しさまでは保証されません。
