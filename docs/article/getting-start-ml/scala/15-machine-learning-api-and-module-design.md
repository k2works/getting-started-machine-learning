...
- `.orNotFound` は、`HttpRoutes[IO]`（当たらない経路があるかもしれない）を `HttpApp[IO]`（必ず応答を返す）に変えます。本シリーズでは `routes` の最後に「どのパスにも当たる」ケースを置いて JSON の 404 を返すので、`.orNotFound` の既定の応答が使われることはありません。F# 版が `choose` のリストの最後に置いた 404 のハンドラーと同じ役割です
...
- 知らない経路の 404 は、`routes` の最後に「どのパスにも当たる」ケース（`case _ => NotFound(...)`）を置いて、ほかの版と同じ `{"detail":"見つかりません"}` を返しています。これを書かないと、`.orNotFound` が返す既定の応答（本文が `Not found` という平文）になります
