...
| Bolt | 言語 | 内容 | 完了条件 |
|------|------|------|---------|
| B50 ウォーキングスケルトン（完了） | Ruby | Nix の `ruby` 環境の確認、`apps/ruby/` の雛形、ADR 010、第 1 章の実装と記事、Ruby 版のトップページ、nav、Ruby CI | 第 1 章のテストが CI でグリーン。記事がサイトで表示される。静的解析のルールが有効になっている（わざと違反を入れて失敗することを確かめる） |
| B51（完了） | Ruby | 第 2〜3 章 | 自作の決定木とライブラリの結果を並べて載せられる |
...
| B60〜B64 | Elixir | 同上（ADR 012、`apps/elixir/`） | Elixir 版の全章完了（2026-09-23 完了） |
| B65〜B69 | PHP | 同上（ADR 013、`apps/php/`） | PHP 版の全章完了（2026-09-24 完了） |
| B70〜B74 | Haskell | 同上（ADR 014、`apps/haskell/`） | Haskell 版の全章完了（2026-09-24 完了） |
| B75（完了） | 統合解説 | `integration/` の各章と索引に第 3 波の 5 言語を加える | 統合解説の各表で 14 言語の行・列がそろっている（2026-09-24 完了） |
...
### B51〜B54 の進め方（2026-09-22、目標「Ruby 執筆完成」による）

2026-09-22 に人から「Ruby 執筆完成」を目標として受け取ったので、B50 の完了後から B54 までを続けて進める。割り当ては Rust 版（B45〜B48）と同じ形にする。
...
| 第 11〜12 章・第 13〜14 章（実装と記事） | サブエージェント | 第 7 章・第 9 章の取り込み後 |
| 第 15 章 | 親 | 第 7・8 章 |
| 索引・nav・執筆計画・ログ・ADR 010 の統合 | 親 | 各章の取り込み |

#### データの表し方と決定木（第 2 章以降のすべての章に効く判断）

| 対象 | 表し方 | 理由 |
| :--- | :--- | :--- |
| 読み込んだ行 | `Chapter02::Row`（セルの文字列の `Hash`）。`number(column)` は空欄なら `nil`、`text(column)` は列が無ければ `KeyError` | Ruby には `Option` が無いので、欠損値は `nil` で表す |
| 表 | `Chapter02::Table = Data.define(:columns, :rows)`。`CSV.read` で読む | `Hash` は挿入の順を保つので、列の順も保てる |
| 補完した後の特徴量 | `Chapter02::Features = Data.define(:columns, :values)`。列名と値の数が違えば `ArgumentError` | `Data` は値で比べられるので、テストでそのまま `assert_equal` できる |
| 分割の結果 | `Chapter02::TrainTestSplit = Data.define(:x_train, :x_test, :t_train, :t_test)` | 動的型付けなので、ジェネリクスは要らない |
| 決定木 | `Leaf = Data.define(:label)` と `Node = Data.define(:split, :left, :right)` を `case`/`in` で見分ける | 判別共用体は無いが、パターンマッチで Rust 版の `match` に近く書ける。網羅性は検査されない |
| Rumale への受け渡し | `Numo::DFloat.cast(x.map(&:values))` と、ラベルを番号にした `Numo::Int32` | Rumale のラベルは整数 |
| 失敗 | `ArgumentError`（値が不正）・`KeyError`（列や値が無い）・`RuntimeError`（学習前の予測） | 例外で表す |

乱数は `Array#shuffle(random: Random.new(seed))` を使う。分かれる行と訓練データの平均値はほかの言語版と一致しない（件数 105 件・45 件は一致する）。第 3 章の深さ 2 の決定木のテストデータの正解率は 0.9556（43/45）で、深さ 1〜5 では Rumale と予測が一致する。深さを制限しないと 1 件分かれる（Rumale は節ごとに特徴量をランダムな順で調べるため）。

#### B51〜B54 の完了記録（2026-09-22）

Ruby 版の全 15 章が完成した（B51〜B54）。実装は `apps/ruby/lib/getting_started_ml/`、記事は `docs/article/getting-start-ml/ruby/`、検査は `npx gulp apps:check:ruby`（`bundle exec rake check`＝RuboCop → Minitest＋SimpleCov）。テストは 330 件で、学習データが無い環境では 38 件がスキップされる。行カバレッジは 98.71%。

| 決めたこと | 結果 |
| :--- | :--- |
| ライブラリと突き合わせた章 | 第 3・7・9〜14 章。Rumale に無い**欠損値の補完・ダミー変数化・クラスの重み（第 8・9 章）、PCA の寄与率と固有値分解（第 13 章）、正規方程式（第 7・9 章）**が自作の最終実装 |
| 数値の一致 | 第 13 章（主成分分析）の寄与率はほかの言語版と表示の桁まで一致した。乱数を使う章は件数だけが一致する |
| 想定外だったこと | **Rumale のモデルは `random_seed` を省くと `srand` でプロセス全体の乱数の種を入れ替える。** `Numo::Linalg` が無いと、`LinearRegression`・`Ridge`・`PCA` は指定した solver を無視して反復法に黙って落ち、既定の `tol` では答えが大きくずれる（第 7 章の既定の決定係数は −1.246）。`StandardScaler` は n−1 で割る。`Ridge`・`Lasso` は切片も正則化する |
| 環境 | Nix の環境で `RUBYLIB` に Solargraph の依存の gem が並び、`bundle exec` が `Gemfile.lock` より古い gem を読んでいた。`shellHook` で `unset RUBYLIB` して直した。改行は `.gitattributes` と RuboCop の `Layout/EndOfLine` の両方で LF にそろえた |
| 第 15 章 | Sinatra 4.2＋Puma、統合テストは rack-test。置き場の約束は、本物と偽物に include する共通のテストのモジュールで表した |
| 進め方 | 割り当ての表と違い、**第 15 章は親ではなくサブエージェントが書いた**（第 7〜8 章の取り込み後に、第 11〜14 章と並行して進めるため）。サブエージェントどうしの衝突は `lib/getting_started_ml.rb` の `require` の行だけで、親がまとめた |

各章で確かめたライブラリの癖は [ADR 010](../../adr/010-ruby-ml-libraries.md) の「各章で確かめた結果」に記録した。

**Ruby 版が完了した。** シリーズは 10 言語になった。第 3 波の次の言語は Clojure（B55〜）。
...
### B56〜B59 の進め方（2026-09-23、目標「Clojure 執筆完成」による）

2026-09-23 に人から「Clojure 執筆完成」を目標として受け取ったので、B55 の完了後から B59 までを続けて進める。割り当ては Ruby 版（B51〜B54）と同じ形にする。
...
| 索引・nav・執筆計画・ログ・ADR 011 の統合 | 親 | 各章の取り込み |

#### データの表し方と決定木（第 2 章以降のすべての章に効く判断）

| 対象 | 表し方 | 理由 |
| :--- | :--- | :--- |
| 読み込んだ行 | 列名のキーワードから文字列へのマップ。`number` は空欄なら `nil`、`text` は列が無ければ例外 | Clojure には Option が無いので、欠損値は `nil` で表す |
| 表 | `{:columns [キーワード...] :rows [行...]}` | 列の順を保つ（マップの鍵の順には頼らない） |
| 補完した後の特徴量 | 列名のキーワードから `double` へのマップ | 値で比べられるので、テストでそのまま `=` で確かめられる |
| 分割の結果 | `{:x-train ... :x-test ... :t-train ... :t-test ...}` | 動的型付けなので、ジェネリクスは要らない |
| 決定木 | 葉は `{:label ...}`、節は `{:split ... :left ... :right ...}`。`leaf?`・`node?` で見分ける | 判別共用体が無いので、鍵があるかどうかで見分ける。網羅性は検査されない（Scala 3 の `enum` や Rust の `enum` との対比） |
| Tribuo への受け渡し | `ArrayExample`・`MutableDataset`・`CARTClassificationTrainer` を Java の相互運用で呼ぶ。列名は `String` の配列 | Java 版・Kotlin 版・Scala 版と同じ 4.3.2 |
...
B60（Elixir のウォーキングスケルトン）は 2026-09-23 に完了した。第 1 章の正解率は 0.7368 で、ほかの言語版と一致した。

- **Scholar に決定木とランダムフォレストが無かった。** 本書の背骨が丸ごと無いので、突き合わせは線形回帰・ロジスティック回帰・K-means・主成分分析・評価指標・前処理に限られる。**ほかのどの言語版よりも置き換えの範囲が狭い**。「ライブラリが育っていない領域では、自作がそのまま本番の実装になる」ことを正面から扱う版になった
- **乱数を自作して JVM の 4 言語版と並びをそろえた。** `Nx.Random`（Threefry）も Erlang の `:rand` も合わないので、`java.util.Random` と同じ 48 ビットの線形合同法を書いた。シード 0 の並びが `[4, 8, 9, 6, 3, 5, 2, 1, 7, 0]` になり、第 2 章の訓練データの平均（0.4215384615384616）も 1e-15 まで一致した。ライブラリが無いからではなく、**数値を突き合わせるために自作する**という選択
- **Nx のテンソルは既定が f32。** `type: :f64` を明示しないと重回帰の係数が `0.9999998807907104` になる。NumPy が既定で f64 なのとちょうど裏返し
- **Elixir のマップはキーの順を保たない。** 列の順は CSV から取ったリストで持ち回る（Clojure 版の「9 件目から崩れる」と同じ落とし穴）
- **`mix test --cover` のしきい値は `test_coverage: [summary: [threshold: N]]` に書く。** `[threshold: N]` と直に書くと**黙って無視され**、既定の 90% のまま失敗し続ける。同じ `test_coverage` の `ignore_modules` は効くので気づきにくい
- **`NimbleCSV.define/2` が生成するモジュールがカバレッジの集計に入る。** 外さないと総計が大きく落ちる
- `erl` は OTP 28 だが、Elixir は自分が持つ **OTP 27** で動く（Clojure 版の「`java` は 25 だが Clojure は JDK 21」と同じ形）
- 検査の終了コードがばらばら（整形 1・警告 1・**Credo 4**・テスト 2・カバレッジ 3）。4 つとも実際に壊して落ちることを確かめた。**未使用の変数は Credo ではなくコンパイラが警告する**ので、`mix compile --warnings-as-errors` を組み合わせる
- **`git grep` は未追跡ファイルを見ない。** 新規作成した記事の BOM の検査で 0 件と誤判定した。`git add` してから検査するか、素の `grep -rP` を使う
...
#### B61〜B64 の完了記録（2026-09-23）

Elixir 版の全 15 章が完成した（B61〜B64）。実装は `apps/elixir/lib/getting_started_ml/`、記事は `docs/article/getting-start-ml/elixir/`、検査は `npx gulp apps:check:elixir`（`mix format --check-formatted` → `mix compile --warnings-as-errors` → `mix credo --strict` → `mix test --cover`）。テストは 321 件で、学習データが無い環境では 31 件が外れる。カバレッジは 94.80%。

| 決めたこと | 結果 |
| :--- | :--- |
| ライブラリと突き合わせた章 | 第 7・9〜14 章。**Scholar に無い決定木・ランダムフォレスト（第 3・8・10 章）・ラッソ（第 12 章）・グループ別の補完とダミー変数の基準列の削除（第 8 章）・クラスの重み（第 8 章）**が自作の最終実装。**置き換えの範囲はシリーズで最も狭い** |
| 数値の一致 | **第 2〜14 章の数値が Java 版・Scala 版・Clojure 版と一致した。** 自作の乱数（`java.util.Random` と同じ線形合同法）が効いている。一致しなかったのは 2 か所だけで、第 10 章の特徴量の重要度（同点の分割 269 回で `Enum.frequencies/1` の走査順が違い 1 ulp 崩れる。正解率は一致）と、第 15 章の API の応答（`7730.457421687016` 対 `7730.457421687023`。正規方程式を解く実装の丸めの順） |
| Scholar の癖 | **`LinearRegression` は pinv（SVD）で解くので条件の悪いデータで最小二乗解に届かない**（第 7 章。残差平方和で自作が勝つ）。同じ原因が `RidgeRegression` では `solver: :cholesky` という選べる形で露出していて、切り替えると自作と 1 ビットも違わない（第 12 章）。**分類の指標は f64 を渡しても f32 を返す**（第 11 章）。`k_fold_split/2` は余りを捨てる（第 11 章）。`KMeans` は初期中心を渡せない（第 14 章）。**`Nx.LinAlg.eigh` は対称でない行列を黙って受ける**（第 13 章） |
| 性能 | **EXLA を入れない Nx は、この規模では素の Elixir より遅い。** 勾配降下法で 7 倍（第 10 章）、K-means で 30 倍以上（第 14 章） |
| 第 15 章 | Plug＋Bandit＋Jason。置き場の約束は behaviour だが、behaviour はモジュールへの約束なので置き場は `{モジュール, 状態}` の組になる（Plug 自身の `plug: {Api, store}` と同じ形）。振る舞いの約束は本物と偽物の両方に走らせるテストで守る。`:erlang.term_to_binary` の往復は完全一致する |
| 進め方 | 第 2〜3 章の実装は親、ほかはサブエージェント。第 13 章が第 9 章と標準化・ダミー変数化を重複して実装していたので、親が第 9 章に寄せた（実装・テスト・記事の 3 つ） |

各章で確かめた癖は [ADR 012](../../adr/012-elixir-ml-libraries.md) の「各章で確かめた結果」に記録した。

**Elixir 版が完了した。** シリーズは 12 言語になった。第 3 波の次の言語は PHP（B65〜）。
...
### B66〜B69 の進め方（2026-09-24、目標「PHP 執筆完成」による）

2026-09-24 に人から「PHP 執筆完成」を目標として受け取ったので、B65 の完了後から B69 までを続けて進める。割り当ては Elixir 版（B61〜B64）と同じ形にする。

| 範囲 | 担当 | 前提 |
| :--- | :--- | :--- |
| 第 2 章の実装（自作の乱数・CSV・前処理） | 親 | — |
| 第 2 章の記事 | 親 | 第 2 章の実装 |
| 第 3 章（実装と記事） | サブエージェント | 第 1〜2 章 |
| 第 4〜6 章（実装と記事） | サブエージェント | 第 1〜2 章 |
| 第 7〜8 章（実装と記事） | サブエージェント | 第 2 章 |
| 第 9〜10 章（実装と記事） | サブエージェント | 第 2 章 |
| 第 11〜12 章・第 13〜14 章・第 15 章（実装と記事） | サブエージェント | 第 7 章・第 9 章の取り込み後 |
| 索引・nav・執筆計画・ログ・ADR 013 の統合 | 親 | 各章の取り込み |

第 2 章で確かめたこと（2026-09-24）。

- **`java.util.Random` と同じ 48 ビットの線形合同法を自作した。** `mt_rand` はほかの言語版と並びが合わない。`shuffle(0..9, 0)` が `[4, 8, 9, 6, 3, 5, 2, 1, 7, 0]` になり、Java 版・Kotlin 版・Scala 版・Clojure 版・Elixir 版と一致した。訓練データのがく片長さの平均（0.4215384615384616）も 1e-15 まで一致した
- **PHP の整数は溢れると float に化ける。** `state * 0x5DEECE66D` は 83 ビットになるので、素直に書くと精度を失って並びが崩れる（Java のように折り返さない。`Deprecated: Implicit conversion from float ... loses precision` が出て初めて気づいた）。state を上下 24 ビットに分け、どちらも 59 ビットに収まる掛け算にしてから 48 ビットで足し合わせる
- **`fgetcsv` は `escape: ''` を明示する。** PHP の CSV 関数は歴史的にバックスラッシュのエスケープを解釈し、RFC 4180 と違う結果になる。PHP 8.4 で非推奨になり、明示すれば標準どおりになる
- **PHPStan のレベル 9 は配列の値の型まで要求する。** `array $row` と書くだけでは通らず、`@param array<string, string> $row` が要る。`list<T>` を返すと宣言した関数は、添字を書き換えたあとに `array_values()` で包み直さないと「list とは限らない」と指摘される
- **テストの書き方も静的解析に合わせる必要がある。** `assertIsFloat($value)` は型注釈から常に真と分かるので指摘される。表明は「型」ではなく「値」を確かめる形に書き換える

#### B66〜B69 の完了記録（2026-09-24）

PHP 版の全 15 章が完成した（B66〜B69）。実装は `apps/php/src/`、記事は `docs/article/getting-start-ml/php/`、検査は `npx gulp apps:check:php`（PHP-CS-Fixer → PHPStan レベル 9 → PHPUnit → 自作のカバレッジしきい値）。テストは 360 件で、学習データが無い環境では 34 件がスキップされる。行カバレッジは 99.17%。

| 決めたこと | 結果 |
| :--- | :--- |
| ライブラリと突き合わせた章 | **第 3・7〜14 章。Elixir 版と正反対に、置き換えの範囲がシリーズで最も広い部類になった。** 自作が最終実装になったのは、ラッソ（第 12 章・Rubix ML に無い）、ROC 曲線と AUC（第 11 章・同）、クラスの重み（第 8 章・口が無い）、グループ別の補完とダミー変数の基準列の削除（第 8 章・同）、15×15 の固有値分解（第 13 章・MathPHP が落ちる） |
| 数値の一致 | **第 2〜14 章の数値が Java 版・Scala 版・Clojure 版・Elixir 版と一致した。** 自作の乱数（`java.util.Random` と同じ線形合同法）が効いている。**第 10 章の特徴量の重要度は Elixir 版だけがずれていた箇所で、PHP は Java 版・Clojure 版と完全に一致した**（PHP の連想配列が挿入順を保つため）。一致しなかったのは第 15 章の API の応答だけで（`7730.457421687019` 対 JVM 版 `…023`・Elixir 版 `…016`）、原因は正規方程式を解く実装の丸めの順 |
| **「ある」と「そのまま比べられる」は別** | **この版でいちばん大きい発見。** `ClassificationTree` は `array_rand` で列を選び**シードを渡す口が無い**ので、既定のままでは実行ごとに正解率が揺れる（5 回で `0.8889 ×4, 0.8222`）。`RandomForest` と `SoftmaxClassifier` も大域の `mt_rand` から引く。第 3 章は `maxFeatures` を列数と同じにしてくじ引き自体を起こさず、第 10 章は `fit()` の先頭で `mt_srand()` を呼ぶ——**同じ問題に 2 つの章が別の手で対処する**形になった |
| Rubix ML の癖 | `Metric` は「大きいほどよい」に統一されているので**誤差指標は負値を返す**（第 11 章）。**`FBeta` は 2 値でもマクロ平均**（同）。**`Labeled::fold()` は余りを捨てる**（同。Scholar と同じ）。**`Reports\ConfusionMatrix` は行が予測・列が正解**（教科書とは逆向き。同）。**`LogisticRegression` は 2 クラス専用**（第 10 章）。**`PrincipalComponentAnalysis` は `lossiness()` しか公開していない**が、`transform()` が線形変換であることと `lossiness()` の差分から主成分も寄与率も復元できた（第 13 章）。**`KMeans` はミニバッチ K-means** なので初期中心を渡しても一致しない（第 14 章） |
| 型を使う選択 | Ruby 版が RBS・Steep を使わないと決めたのと逆に、`declare(strict_types=1)` と PHPStan のレベル 9 を使い切った。**実行時に効く型と静的解析だけが見る型の二層構造**が、この版の背骨になった。代償も出た——関数を返す関数の PHPDoc が書きにくく（第 12 章）、約束のテストに `assertIsFloat` と書くと「常に真である」と叱られる（第 15 章）。**そして型の道具をどれだけ使っても、PHP の配列が「数字だけの文字列」の鍵を整数に変える癖は型では守れない**（第 3 章で自分のコードに、第 11 章では Rubix ML のほうが落ちた） |
| 第 15 章 | 置き場の約束は **interface**。Elixir 版が behaviour のために `{モジュール, 状態}` の組を必要とした回り道が消え、偽物のモデルは無名クラスでその場に書ける。**組み込みサーバーはプログラムから起動できない**（`php -S` 自体がサーバー）ので学習・保存と待ち受けがコマンドとして分かれ、結果として組み立てのクラスまで 100% テストできた（Elixir 版の `Chapter15` は 50%） |
| 環境 | **素の Nix 環境にカバレッジドライバ（xdebug・pcov）が無く、環境定義に手を入れた。** ほかの言語版では起きなかった前提の崩れ。あわせて `php`（8.4）と `php83Packages.composer`（8.3）の版ずれも解消した。**PHPUnit には最低カバレッジのしきい値の機能が無い**ので、clover の XML を読む判定を自作した |
| 進め方 | 第 2 章の実装と記事は親、ほかはサブエージェント。**第 9 章と第 7 章が `LinearModel` を重複して作っていたので、親が第 7 章に寄せた**（Elixir 版で第 13 章が第 9 章と標準化を重複した件と同じ形。第 13〜14 章には事前に「標準化を作り直すな」と伝えて再発を防げた）。**第 11〜12 章のテストにデータ不在時のスキップが漏れていた**（`#[Group('data')]` はあるが `markTestSkipped` が無く、学習データの無い CI で 10 件が落ちる状態だった）ので、親がヘルパーを足して直した |

各章で確かめた癖は [ADR 013](../../adr/013-php-ml-libraries.md) の「各章で確かめた結果」に記録した。

**PHP 版が完了した。** シリーズは 13 言語になった。第 3 波の次の言語は Haskell（B70〜）。
...
### B71〜B74 の進め方（2026-09-24、目標「Haskell 執筆完成」による）

B70 の完了後から B74 までを続けて進める。割り当ては PHP 版（B66〜B69）と同じ形にする。
...
| 第 9〜10 章（実装と記事） | サブエージェント | 第 2 章 |
| 第 11〜12 章・第 13〜14 章・第 15 章（実装と記事） | サブエージェント | 第 7 章・第 9 章の取り込み後 |
| 索引・nav・執筆計画・ログ・ADR 014 の統合 | 親 | 各章の取り込み |

第 2 章で確かめたこと（2026-09-24）。

- **`Int` は Java と同じく溢れると折り返す**ので、線形合同法の式をそのまま書ける。`nextInt 100` の並び `[60, 48, 29, 47, 15]` と `shuffle [0..9] 0` の `[4, 8, 9, 6, 3, 5, 2, 1, 7, 0]` が Java 版・Elixir 版・PHP 版と一致し、訓練データのがく片長さの平均（0.4215384615384616）も一致した
- **不変のリストでは添字の書き換えができない**ので、Fisher-Yates は `Map` に写してから交換する。乱数の状態は引数と戻り値で受け渡すので、同じ状態からは必ず同じ値が出る（大域の状態を持たないぶん、PHP 版の `mt_srand` のような取り扱いが要らない）
- **欠損値は `Maybe` で表す。** 取り出す前に場合分けを書くことになり、扱いを忘れられない
- **テストの中でも `let Right x = ...` は書けない。** 網羅していないパターンとして `-Wincomplete-uni-patterns` が `-Werror` で止める。`expectRight` のような関数にして「失敗したらその場で落ちる」ことを明示する
- **`Map` はキーの順で並ぶ**ので、CSV に現れた列の順は別のリストで持ち回る（Elixir 版・Clojure 版と同じ扱い）
- 1 列だけの CSV では「空欄の行」と「空行」を区別できない（空行は読み飛ばすため）。テストのデータは 2 列以上にする

#### B71〜B74 の完了記録（2026-09-24）

Haskell 版の全 15 章が完成した（B71〜B74）。実装は `apps/haskell/src/GettingStartedMl/`、記事は `docs/article/getting-start-ml/haskell/`、検査は `npx gulp apps:check:haskell`（fourmolu → hlint → `cabal test --enable-coverage --enable-executable-dynamic` → 自作のしきい値判定）。テストは 375 件で、学習データが無い環境では 41 件が pending になる。式カバレッジは 93%（7676/8206）。

| 決めたこと | 結果 |
| :--- | :--- |
| ライブラリと突き合わせた章 | **第 7・9・12・13 章だけ。シリーズで最も狭い。** 機械学習のライブラリが無いので、決定木（第 3 章）・前処理（第 8 章）・ロジスティック回帰とランダムフォレスト（第 10 章）・**評価指標と交差検証（第 11 章）**・ラッソ（第 12 章）・K-means（第 14 章）はすべて自作が最終実装。**とくに第 11 章は「突き合わせの節が存在しない」シリーズ唯一の章**になり、値の分かっている入力・満たすべき性質・ほかの言語版の数値の 3 つで自作の妥当性を支えた |
| 数値の一致 | **第 2〜14 章の数値がほかの言語版と一致した。** 自作の乱数（`java.util.Random` と同じ線形合同法）が効いている。**`Int` が Java と同じく折り返すので、PHP 版で必要だった乗算の分割が要らなかった。** 一致しなかったのは 2 か所——第 10 章の特徴量の重要度（**`Data.Map` が鍵の順で畳むため Elixir 版と一致し、挿入順の Java 版・Clojure 版・PHP 版とは 1 ulp ずれた**。PHP 版が「Elixir 版だけずれた」と書いた箇所の裏返し）と、第 15 章の API の応答（`7730.457421687022`。**第 3 波の 3 版ではいちばん JVM の …023 に近い**） |
| **C ライブラリの確認は 3 段ある** | **この版でいちばん大きい発見。** (1) ビルドが通るか、(2) 呼び出しが通るか、(3) 環境を変えたあとに作り直されるか——は別物。**B70 のステップ 1 では 1 つ目しか確かめずに hmatrix の採用を決めていた。** 実際には `openblas` が ILP64 で 32 ビット整数を渡す hmatrix と ABI が噛み合わず、**ビルドは通るのに 2×2 の連立方程式すら解けなかった**。`blas` + `lapack`（LP64）に差し替えて解決したが、**`LIBRARY_PATH` は cabal のパッケージハッシュに含まれない**ので store の作り直しも要った |
| hmatrix の癖 | **`<\\>` は SVD（DGELSS）で解く。** 同じ実データを 4 通りで比べると、自作のガウス・ジョルダン法・`<\\>`（計画行列）・`linearSolveLS` の 3 つが 12 桁一致し、**`<\\>` に正規方程式を渡したものだけが 9 桁に落ちる**（`XᵀX` で条件数が 2 乗になる教科書どおりのことが、同じ関数に別の入れ物で渡しただけで出た）。**`eigSH` は 15×15 の実データに耐えた**（PHP 版で MathPHP が落ちた場所）。**`trustSym` は対称性を確かめない**（Elixir 版の `Nx.LinAlg.eigh` と同じ落とし穴） |
| 型を使い切る | 失敗は `Either`、欠損は `Maybe`、取りうる値は直和型で数え上げ、`-Wall -Werror` で網羅漏れを止めた。**テストの中でも失敗を無視できない**（`let Right x = ...` が `-Wincomplete-uni-patterns` に止められ、`expectRight` で「落ちること」を明示した）。**`accuracy` を `(Eq a) =>` に広げたら、広げる前には書けていた `accuracy [] []` が型の曖昧さで書けなくなった**——どこまで抽象するかは設計の判断である |
| 第 15 章 | 置き場の約束は**レコード**（型クラス版も書いて比較したうえで選んだ）。型クラスは 1 つの型に 1 つのインスタンスなので偽物に型が 2 つ要り、制約が全層に伝播する。**`IO` を境界に閉じ込める構造が、そのまま層の分離になった。** 失敗が直和型なので**ステータスコードへの変換をコンパイラが確かめる** |
| 環境 | **素の Nix 環境に fourmolu も hlint も無く、手元の `/usr/local/bin` が見えていた**（PHP 版の pcov と同じ形で第 3 波 2 回目）。**cabal にも最低カバレッジのしきい値の機能が無い**ので HPC の結果を読む判定を自作した（第 3 波で 3 回続けてカバレッジの仕組みに手を入れた）。**章が 15 個そろった時点で、カバレッジ付きのテストがリンクで落ちるようになった**（HPC の初期化配列が macOS のリンカの範囲を超える）。`--enable-executable-dynamic` で解決 |
| 進め方 | 第 2 章の実装と記事は親、ほかはサブエージェント。**共有資源で 2 つの問題が出た**——`getting-started-ml.cabal` の依存漏れ（`scientific`）で全員のビルドが止まり、担当外の報告で解消した。また **cabal はビルドディレクトリの排他制御をしない**ので、複数が同時に `cabal test` を走らせて `dist-newstyle` が壊れた。以降は親がまとめて 1 回だけ検査する形にした。重複は 1 件（`accuracy`）を第 1 章に寄せた |

各章で確かめた癖は [ADR 014](../../adr/014-haskell-ml-libraries.md) の「各章で確かめた結果」に記録した。

**Haskell 版が完了した。シリーズは 14 言語になり、第 3 波のすべての言語がそろった。** 残るは B75（統合解説に第 3 波の 5 言語を加える）。
...
### B75 の完了記録（2026-09-24）

多言語統合解説（`integration/`）の索引と全 5 章を 14 言語に拡張した。

| 章 | 主な更新 |
| :--- | :--- |
| 索引 | 対象言語一覧を波ごとに 3 表・14 行へ（縦持ちに組み替え）。「第 3 波を追加したら更新します」の記述を削除 |
| 1 言語とライブラリの概要 | 道具立て・パッケージ管理・ライブラリの表を波ごとに拡張。**新設「カバレッジの仕組みは言語ごとにばらばらだった」**（Elixir は既定のしきい値あり・PHP はドライバすら無い・Haskell は仕組みはあるがしきい値が無く、しかも章が増えるとリンクが落ちる）。**新設「環境に道具が無く、手元の PATH のものが見えていた」**（PHP・Haskell・Ruby・Clojure の 4 言語で起きた） |
| 2 データ構造の比較 | **乱数の節を全面的に書き直した。** 第 2 波までの「代わりが無いから自作した」（TypeScript）と、第 3 波の「代わりはあるが、数値を突き合わせるために自作した」（Elixir・PHP・Haskell）を対比。9 言語 7 通り → **14 言語 8 通り**で、`java.util.Random` の並びを共有する **6 言語が第 3 章の 6 行すべて一致**する。**整数の溢れが 4 通り**（PHP だけ float に化ける）・列の順を別に持ち回った 3 言語・和を積む順で 1 ulp ずれる件・丸めの向き（PHP と Haskell が偶数側）を新設。BOM・Shift_JIS・欠損値の表を 14 言語に |
| 3 アルゴリズムの実装の比較 | 木の型・行列・共通の型・遅延評価の表を波ごとに 3 分割。網羅性の 4 段階に **Haskell を段階 1、PHP を段階 3（型で書けるが網羅性は強制しない）** として配置。**Haskell が第 10 章（型クラス＋存在型）・第 11 章（ただの関数）・第 15 章（レコード）で 3 通りを使い分けた**ことを扱った。「遅延だから 1 件」が 2 回裏切られた件（Clojure の 32 件チャンク、Haskell の `traverse`）を節に。置き場の約束の表（14 通り）を新設 |
| 4 ライブラリのエコシステムの比較 | 章ごとの突き合わせ表に第 3 波を追加し、置き換えの範囲を 5 段階の縦持ちに整理。**新設「『ライブラリにある』と『そのまま比べられる』は別」**。**「決定的でない」を大幅拡張**（Rubix ML の 3 か所・Rumale・Scholar）。**新設「C ライブラリへの依存——確認すべきことが 3 段ある」**。**新設「同じ計算を違う入れ物で渡すと精度が変わる」**（Elixir・PHP・Haskell の「SVD で解いた」が 3 通り）。**新設「ライブラリが無いことが安全な設計を強制した例」** |
| 5 学習ロードマップ | 目的別の表に 5 言語を追加。読み比べの順を 4 → 8 に拡張し、**ライブラリの有無・型の使い方・同じ数値**の 3 軸に分類。「これからの版」を第 3 波完了に書き直し、「波ごとに新しく見えた軸」の表を新設 |

**あわせて、シリーズ全体にまたがる事実誤認を 1 件直した。** 自作の線形合同法の並びが「Java 版・**Kotlin 版**・Scala 版・Clojure 版と一致する」という記述が ADR 2 本と記事 16 ファイル・実装のコメント 5 か所に広がっていたが、**Kotlin 版は `kotlin.random.Random`（XorWow）を使っており一致しない**（第 3 章の正解率が深さ 2 で 0.9429/0.9333 対 0.9333/0.9556）。Elixir 版は章によって矛盾しており、第 2 章で「一致する」と書きながら第 9・11 章では「乱数が別実装の Kotlin 版とは一致しません」と正しく書いていた。統合解説の担当が実測から見つけて報告した。

**B75 が完了し、第 3 波のすべての Bolt（B50〜B75）が終わった。シリーズは 14 言語 × 15 章で完成した。**
