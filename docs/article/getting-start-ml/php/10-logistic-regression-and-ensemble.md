...
実は、[第 3 章](03-decision-tree-and-obvious-implementation.md) も同じ非決定性に当たっています。`ClassificationTree` は分割のたびに使う列を `array_rand()` で選ぶので、既定のままでは実行ごとに結果が変わります。第 3 章の対処は **`maxFeatures` を列数と同じにして、くじ引きそのものを起こさない** ことでした。この章の森は列を絞ることが前提の道具なので、同じ手は使えません。そこで種付けのほうを選びました。

**同じ「ライブラリがシードを受け取らない」という問題に、2 つの章が別の手で対処している** ことになります。くじ引きが本質でないなら消す、本質なら種を固定する。どちらも「大域の乱数に依存している」という事実は消えないので、**ライブラリが乱数を使う場所を把握していないと、どちらの手も選べません**。
