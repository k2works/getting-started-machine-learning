...
- **`OK (360 tests, 3772 assertions)`** — 表明の数がテストの数よりずっと多いのは、乱数の範囲を 1000 回確かめるような繰り返しのテストがあるからです（第 2 章）
- **`行カバレッジ: 99.17% (2391/2411)`** — しきい値 75% に対して余裕があります。数値は章が増えるたびに動きます

上の数値は、この章を書いた時点のものです。章を書き進めればファイルもテストも増えるので、**手元で打ったときに同じ数字が出るとは限りません**。一致すべきなのは、正解率のような **アルゴリズムの結果** であって、ファイルの数ではありません。

### しきい値は 90% から 75% に下げた

[第 1 章](01-machine-learning-and-first-test.md)では、しきい値を 90% にしていました。いまは 75% です。**下げた理由は、CI が必ず落ちるようになったからです。**

```text
行カバレッジ: 78.35% (1889/2411), しきい値: 90.00%
カバレッジがしきい値を下回りました: 78.35% < 90.00%
##[error]Process completed with exit code 3
```

手元では 99.17% なのに、CI では 78.35% です。差は **学習データの有無** から来ています。配布データはリポジトリに置けないので（[第 4 章](04-version-control-and-data-management.md)）、CI では実データのテストが `markTestSkipped` で飛ばされます。飛ばされたテストが通っていたはずの行は、カバレッジに数えられません。

| 環境 | 実データのテスト | 行カバレッジ |
|------|---------------|-----------|
| 手元（`apps/data/sukkiri-ml/` にデータあり） | 走る | 99.17% |
| CI（データ無し） | 34 件がスキップ | **78.35%** |

**しきい値は「どの環境でも保てる水準」でなければ意味がありません。** 高いほうに合わせると CI が必ず落ち、落ちることが当たり前になって、本当の失敗を見落とします。低いほうを基準にしました。

この失敗は、**CI を動かして初めて分かりました**。手元では最後まで 99% だったからです。第 1 章の時点では「90% に対して余裕があります」と書けていて、それは嘘ではありませんでした。**章が増えて実データのテストが増えるほど、手元と CI の差が開いていった**のです。

数字を決めるときは、それが**どの環境で測られた数字なのか**を一緒に決める必要があります。
  ...
  test:
    ...
    steps:
      ...
      - name: Check coverage threshold
        run: |
          nix develop .#php --command bash -c 'cd apps/php && php tools/coverage-threshold.php build/clover.xml 75'
```
...
- **カバレッジは失敗しうる** — しきい値 75% を下回れば終了コード 3 で失敗します（第 5 章）
