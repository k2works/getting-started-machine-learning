...
PHPStan は `@return array<string, int>` と書いても文句を言いません。**この間違いを見つけたのは静的解析ではなくテストです。** 型の道具をどれだけ使っても、言語そのものの癖は型では守れない、という例になりました。
...
4. **配列は「数字だけの文字列」の鍵を整数に変える** — `array_count_values` でも自分で数えても同じ。`@return array<string, int>` と書いても PHPStan は通してしまい、**この間違いを見つけたのはテストだった**。型の道具をどれだけ使っても、言語そのものの癖は型では守れない
