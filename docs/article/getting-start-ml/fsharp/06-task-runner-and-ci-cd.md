...
```javascript
  {
    ...
    // CI（.github/workflows/dotnet-ci.yml）と同じ順に、整形・静的解析・Notebook の出力・テストを検査する
...
- `setup` と `check` は、`apps/dotnet` で実行するシェルのコマンドです。`&&` でつないでいるので、どれか 1 つが失敗するとそこで止まります
...
| 道具 | 実行する場所 | 受け持つもの |
|------|------------|------------|
| `dotnet` CLI とローカルツール | `apps/dotnet` | F# 版の個々の作業（ビルド・テスト・整形・静的解析・Notebook の出力の検査） |
...
Notebook は `apps/dotnet/notebooks/` に置いています。VS Code の Polyglot Notebooks で開くと、F# のカーネルで実行できます。Notebook からは、`dotnet build` で作ったプロジェクトの DLL を読み込み、テスト済みの関数を呼びます。
...
- `__SOURCE_DIRECTORY__` は、スクリプトのファイルがあるディレクトリ（`apps/dotnet/tools`）です。第 1 章の `Dataset.fs` と同じく、どこから実行しても `notebooks/` の場所が決まります
...
プッシュのたびに品質チェックを自動実行するため、`.github/workflows/dotnet-ci.yml` を用意しています。この章で、スクリプトの静的解析と Notebook の出力セルの検査を加えました。
  ...
  test:
    ...
    steps:
      ...
      - name: Cache NuGet packages
        ...
        with:
          path: ~/.nuget/packages
          key: ${{ runner.os }}-nuget-${{ hashFiles('apps/dotnet/**/packages.lock.json', 'apps/dotnet/.config/dotnet-tools.json') }}
          restore-keys: |
            ${{ runner.os }}-nuget-

      - name: Show the .NET SDK version
        run: nix develop .#dotnet --command bash -c "cd apps/dotnet && dotnet --version"

      # packages.lock.json と食い違う依存があれば失敗させる
      - name: Restore tools and packages
        run: nix develop .#dotnet --command bash -c "cd apps/dotnet && dotnet tool restore && dotnet restore --locked-mode"

      - name: Check formatting
        run: nix develop .#dotnet --command bash -c "cd apps/dotnet && dotnet fantomas --check ."

      # ソリューションに含まれないスクリプト（tools/*.fsx）は別に指定する
      - name: Lint
        run: >-
          nix develop .#dotnet --command bash -c
          "cd apps/dotnet && dotnet fsharplint lint MachineLearning.sln && dotnet fsharplint lint tools/notebooks.fsx"

      # Notebook の出力セルには学習データが含まれうるので、残っていれば失敗させる
      - name: Check Notebook outputs
        run: nix develop .#dotnet --command bash -c "cd apps/dotnet && dotnet fsi tools/notebooks.fsx verify"

      # 学習データは再配布できないため CI には配置しない。実データのテストはスキップされる
      - name: Run tests with coverage
        run: >-
          nix develop .#dotnet --command bash -c
          "cd apps/dotnet && dotnet test --no-restore --coverlet --coverlet-include '[MachineLearning]*'
          --coverlet-output-format cobertura --results-directory TestResults"

      # 実データのテストがスキップされるので、手元より低い値になる。下限は設けず表示だけする
      - name: Show coverage
        run: grep -o -m 1 'line-rate="[0-9.]*"' apps/dotnet/TestResults/*.cobertura.*xml
```
...
| `dotnet test` | ビルドしてテストを実行する | `apps/dotnet` |
| `dotnet fantomas .` | コードを整形する | `apps/dotnet` |
| `dotnet fsi tools/notebooks.fsx strip` | Notebook の出力セルを消す | `apps/dotnet` |
| `dotnet fsi tools/notebooks.fsx execute` | Notebook を実行して動作を確認する | `apps/dotnet` |
| `dotnet list package --vulnerable --include-transitive` | 依存関係の既知の脆弱性を確かめる | `apps/dotnet` |
| `dotnet run --project src/MachineLearning -- chapter03` | 章の `Main.run` を実行する | `apps/dotnet` |
