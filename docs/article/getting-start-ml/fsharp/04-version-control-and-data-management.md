...
`scope` には変更の対象を書きます。本リポジトリでは、F# 版の実装なら `fsharp`、記事シリーズなら `getting-start-ml`、Nix の環境定義なら `nix` のように書きます。F# 版の実装は `apps/fsharp/` にあり、scope も言語名にそろえます。同じ .NET の上で C# 版を作ったときに、どちらの言語の変更かを scope で区別できるようにするためです。
...
`apps/fsharp/.gitignore` では、`dotnet` のコマンドが作るディレクトリに加えて、学習済みモデルの保存先と Notebook のチェックポイントを除外しています。
...
```bash
git check-ignore -v apps/data/sukkiri-ml/iris.csv apps/fsharp/src/MachineLearning/bin/ apps/fsharp/src/MachineLearning/obj/ apps/fsharp/tests/MachineLearning.Tests/bin/ apps/fsharp/TestResults/ apps/fsharp/model/ apps/fsharp/notebooks/.ipynb_checkpoints/ tmp/sukkiri-ml-codes.zip
```

```text
.gitignore:8:apps/data/	apps/data/sukkiri-ml/iris.csv
apps/fsharp/.gitignore:1:bin/	apps/fsharp/src/MachineLearning/bin/
apps/fsharp/.gitignore:2:obj/	apps/fsharp/src/MachineLearning/obj/
apps/fsharp/.gitignore:1:bin/	apps/fsharp/tests/MachineLearning.Tests/bin/
apps/fsharp/.gitignore:3:TestResults/	apps/fsharp/TestResults/
apps/fsharp/.gitignore:4:model/	apps/fsharp/model/
apps/fsharp/.gitignore:5:.ipynb_checkpoints/	apps/fsharp/notebooks/.ipynb_checkpoints/
...
```bash
git check-ignore -v apps/fsharp/src/MachineLearning/packages.lock.json apps/fsharp/.config/dotnet-tools.json
echo $?
```
...
ルートの `.gitattributes` では、`apps/fsharp/` 以下のテキストファイルの改行コードを LF に固定しています。
