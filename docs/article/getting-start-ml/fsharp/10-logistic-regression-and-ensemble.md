...
Notebook は `apps/fsharp/notebooks/chapter10_ensemble_exploration.ipynb` にあります。先に `dotnet build` でプロジェクトをビルドしておきます。Polyglot Notebooks が廃止されていることは、[第 2 章の 2.10 節](02-data-preprocessing-and-triangulation.md) を参照してください。ML.NET を使うので、先頭のセルで ML.NET のパッケージも読み込みます。
