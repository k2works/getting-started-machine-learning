...
既定の設定は、FSharpLint のパッケージの中の `fsharplint.json` です（ローカルツールを入れた環境では `~/.nuget/packages/dotnet-fsharplint/0.27.0/tools/net8.0/any/fsharplint.json`）。これを `apps/dotnet/fsharplint.json` に写し、先頭の `ignoreFiles` に `obj/` を加えました。
