...
`gulp apps:check:fsharp` は、`F# CI`（`.github/workflows/fsharp-ci.yml`）と同じ順に、整形の検査（Fantomas）・静的解析（FSharpLint。ソリューションと `tools/notebooks.fsx`）・Notebook の出力セルの検査・テストを実行します。F# 版の Notebook（`apps/fsharp/notebooks/`）は、`apps/fsharp` で次のコマンドを使って扱います。
