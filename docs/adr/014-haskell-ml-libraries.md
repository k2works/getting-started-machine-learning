...
| 11 | 6 指標（正解率 0.7811・適合率 0.7759・再現率 0.6306・F 値 0.6833・RMSE 405.77・MAE 321.53）と分割サイズ（891 件 5 分割で 179,178,178,178,178。Scholar・Rubix ML が余りを捨てるのに対し自作は先頭に配る）がほかの言語版と一致。**評価指標のライブラリが 1 つも無く、シリーズで唯一「突き合わせの節が存在しない章」になった。** 代わりに「値の分かっている入力」（AUC の 1.0 と 0.5、`t=2a` の MSE 0）・「満たすべき性質」（全行をちょうど 1 度テストする、訓練とテストが重ならない）・「ほかの言語版の数値」の 3 つで支えた。**評価関数も分類器もただの関数で足りた**——第 10 章で型クラスと存在型が要ったところが `type Trainer x t p = ...` の 1 行（部分適用がそのまま「関数を返す関数」になるので、PHP 版の PHPDoc も Elixir 版の無名関数も要らない）。**`Either` は遅延しない**——Elixir 版の `Stream.map` は「取り出した分だけ学習」だったが、`traverse` は全分割を要求する（型で失敗を表すことの引き換え）。**閾値の無限大を `Maybe` で書ける**（ほかの言語版が `:infinity`／`INF` を置いた場所）。**`Map` の鍵は型を選ばない**ので、PHP 版が「配列のキーに float を使えない」ために書き直した ROC のグループ化がそのまま書ける。**`0/0` は `NaN` で落ちない**ので「分母が 0 なら 0」と決めたことがそのままテストになった |
| 12 | 実験表・alpha 10.0・テスト R²（線形 0.5224／リッジ 0.6243）・ラッソが 0 にした 3 列がほかの言語版と一致。**リッジの正規方程式は hmatrix と突き合わせた**（人工データで 1e-9 未満、実データ 9 列・alpha=10.0 で最大差 8.0e-15）。**ラッソには突き合わせる相手が無い**（hmatrix は線形代数の道具で座標降下法を持たない）。**座標降下法は「書き換わる変数が畳み込みの引数になる」**——係数を `Map Int Double` に持ち、第 2 章の Fisher-Yates と同じ形になった。**`-Wname-shadowing` に刺された**（テストの局所変数 `sequential` が `Test.Hspec.sequential` と衝突）。第 7 章の `fit` の衝突とは**逆向き**で、あちらはコンパイルエラーで `hiding` が要り、こちらは警告なので改名で済む（`-Werror` でなければ見逃していた） |
...
| 15 | **置き場の約束はレコードで書いた**（型クラス版も使い捨ての module に書いて実際にコンパイルし、比較したうえで選んだ）。理由は 2 つ——**型クラスは 1 つの型に 1 つのインスタンス**なので、偽物を「モデルを持っている／持っていない」で作り分けるのに型が 2 つ要る（レコードなら引数 1 つ）。そして**型クラスの制約は全層に伝播する**が、レコードは `ModelStore ->` の 1 語で止まる。PHP 版の interface・Elixir 版の behaviour・Clojure 版の protocol と並べると、**Haskell だけが 2 つの選択肢から選んだ**ことになる。**予測値は 7730.457421687022 で、第 3 波の 3 版ではいちばん JVM（…023）に近い**（PHP …019、Elixir …016）。差は約 1e-12 で、分割は完全に一致しており違うのは正規方程式の解き方だけ。**`IO` を境界に閉じ込める構造が、そのまま層の分離になった**（`Service.run` の 5 行で「読み込みだけが `IO`、予測は純粋」が書ける）。**失敗が直和型なので、ステータスコードへの変換をコンパイラが確かめる**（`LoadError` に節を増やせば変換の関数が止まる。PHP の `catch`・Elixir の `rescue` は漏れても通る）。**遅延評価がクロージャの代わりになった**（検証で「理由があるときに値を作らせない」ために他版はクロージャを渡していたが、第 2 引数にそのまま書ける）。**aeson の `Value` はオブジェクトと配列を取り違えようがない**ので、PHP 版の `array_is_list` にあたる判定が要らない。**`ByteString` のリテラルに日本語を書きかけたのはこれで 3 回目**（第 1 章の実装・テスト、そしてここ）。`aeson` はキーを並べ替えるので `models` が `status` より先に出る |
| **カバレッジは静的リンクでは取れなかった** | **章が 15 個そろった時点で、`cabal test --enable-coverage` がリンクで落ちるようになった**（`initializer '...Chapter10Spec_init__hpc' is >4GB from start of image`）。HPC が各モジュールに仕込む初期化配列が増えて、macOS のリンカが扱える範囲を超えたため。**`--enable-executable-dynamic`（動的リンク）で解決した。** カバレッジ無しの `cabal test` は 375 件すべて通っていたので、テストの問題ではなくリンクの問題だと切り分けられた。**章が増えるまで現れない種類の失敗**で、第 1 章の時点では起きなかった | 全 15 章がそろった時点で発覚 |
| **しきい値は規模が変わると測り直しが要る** | **カバレッジのしきい値 80% は、章が 15 個そろったところで CI が落ちた。** 第 4 章の時点（663 式）ではデータ無し 83%・有り 94% だったが、全 15 章（8206 式）では**データ無しだけが 71% まで落ちた**（有りは 93% でほぼ変わらない）。実データを使うテストが増えたぶん `pending` の穴が広がったため。「いちばん条件の悪い環境を基準にする」という原則は正しかったが、**その環境の数字自体が動いた**。65% に下げた | 全 15 章の完成後、CI の失敗で発覚 |
| **文字コードの取り違えの振る舞いは OS で変わる** | **`mkTextEncoding` は OS の iconv をそのまま使う。** UTF-8 を `CP932` として読むと、macOS（BSD iconv）は通して文字化けし、**Linux（glibc iconv）は `cannot decode byte sequence starting from 140` で例外にする**。**手元で通ったテストが CI の Linux で落ちて分かった**。環境によらず言えるのは「正しくは読めない」ことだけなので、テストはその形に書き直した | 同上 |
...
| カバレッジ | HPC（`cabal test --enable-coverage --enable-executable-dynamic`）。**しきい値は自作のスクリプトで判定する**（65%。学習データの無い CI で保てる水準にそろえた） | GHC 組み込み | BSD-3-Clause | 第 1 章（記事での解説は第 5 章） |
