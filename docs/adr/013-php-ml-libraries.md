...
| **しきい値は環境ごとに測り直す** | **カバレッジのしきい値 90% は CI で必ず落ちた。** 手元（配布データあり）は 99.17% だが、**CI には学習データが無く実データのテストが 34 件スキップされるので 78.35% まで落ちる**。しきい値は「どの環境でも保てる水準」でなければ意味がないので 75% に下げた。**CI を動かして初めて分かった**（手元では最後まで 99% だった） | 全 15 章の完成後、CI の失敗で発覚 |
...
| カバレッジ | pcov ＋ PHPUnit の clover 出力。**しきい値は自作のスクリプトで判定する**（75%。学習データの無い CI で保てる水準にそろえた） | pcov 1.0 | BSD-3-Clause | 第 1 章（記事での解説は第 5 章） |
