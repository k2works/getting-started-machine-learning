...
ライブラリは `apps/node/package.json` に正確な版で追加する（第 3 章までは使う章に入ってから、第 7〜15 章の分は並行して書く B16 の前にまとめて追加した）。型定義を同梱しないパッケージには、プロジェクトの中に使う範囲だけの型宣言（`declare module`）を書く。
...
| 8 | 重み付けなし・前処理後のデータで ml-cart と突き合わせると、深さ 1〜6 は 179 件すべて一致し、深さ 7〜10 では 8・6・9・6 件の予測が違った（境界ちょうどの値に当たった行は 0 件、同数の葉に落ちた行は深さ 9 で 2 件・深さ 10 で 4 件、残りは分割の選び方の違いと見られる）。第 3 章の `DecisionTree` とは深さ 1〜10 で一致した。重み付けの効果は自作で示し、深さ 5 でテストデータの生存者 67 人のうち見つけた人数が `none` の 40 人から `balanced` の 50 人に増えた。モデルは JSON で保存し zod で検証する。`Map` は JSON で `{}` になるので `Record<string, number>` を使い、スキーマを `z.ZodType<FittedPipeline>` と注釈して型との食い違いを型チェックに見つけさせる |
| 9 | ml-matrix 6.15.0 の `center("column").scale("column")` で標準化できるが、`scale` と `standardDeviation` の既定は不偏標準偏差（件数 − 1 で割る）で、`{ unbiased: false }` で件数で割る。自作は scikit-learn と同じく件数で割るので、置き換えずに自作を最終実装にした。`CholeskyDecomposition#isPositiveDefinite()` で列が独立でない正規方程式を検出できた。`Map.groupBy` は Node.js 22 で動くが `lib: ES2023` では `TS2550` になるので手書きで集計した。`TextDecoder("shift_jis")` で読めるが書き出しはできず、csv-parse は Shift_JIS のバイト列を例外なしに U+FFFD に化かす |
| 10 | ml-logistic-regression 2.0.0 には切片の項が無く（重みは特徴量の数だけ）、iris のテストデータで 26/45 にとどまるが、値が 1 の列を足すと 43/45 になった（自作のソフトマックスは 42/45）。勾配は平均でなく合計で、既定は 50000 回・学習率 5e-4 のため 1 回の学習に約 2 秒かかる。ml-random-forest 2.1.0 は `noOOB: true` なら木が少なくても学習でき、`maxFeatures` は 0 より大きく 1 以下なら割合として扱うので `1` は 100% を意味する。既定の `replacement: true` は特徴量を重複を許して選ぶ。`featureImportance()` は同梱の型定義に無く、計算式も自作と違う。`replacement: false`・`noOOB: true`・内側の ml-cart を `minNumSamples: 1`・`gainThreshold: 0` にそろえても乱数の生成器が違うので木は一致しない（iris のテストデータで ml-random-forest 44/45、自作 42/45） |
| 11 | ml-confusion-matrix 2.0.0 は同じ予測なら件数・正解率・適合率・再現率・F 値が自作と一致した。正例を一度も予測しないと適合率は NaN（再現率・F 値は 0）、無いラベルを正例に指定すると例外になる。`fromLabels(["0","0"], …)` は型引数が `"0"` に推論されるので `fromLabels<string>` と書く。ml-cross-validation 1.3.0 は関数を渡す形の引数の順序が `kFold(features, labels, k, callback)` で README に例が無く、`getFolds` は `Math.random` を使う（`vi.spyOn(Math, "random")` で再現）。余りは最後の分割に足され、余りが分割数以上だと行を捨てる（11 件 4 分割でテストされるのは 9 件）。分割ごとのスコアは返さず全分割の予測を 1 つの混同行列に足し合わせる。Survived・5 分割の ml-cart は深さ 1〜2 で自作と一致し、深さ 3・4 で 1 件、制限なしで 55 件違った。回帰の指標は ml.js に無いので突き合わせを省略した |
...
| 15 | Hono 4.13.8 の `c.req.json<T>()` の型引数は型を宣言するだけで検証しない（検証前は数値の項目に文字列を送っても 200）。壊れた JSON は `SyntaxError` になり、既定の `errorHandler` が 500 を返すので、`app.onError` でモデルが無いことを 503 に変換した。zod 4.6.5 は `z.literal([1, 2, 3])` で値の候補を並べられ、メッセージは `{ error: "..." }` で指定する（既定は英語）。`.nullish()` と `.transform` で JSON の項目名をドメインの型に写し、`z.ZodType<T, unknown>` と注釈すると transform 付きのスキーマも同じ関数で受け取れる。@hono/node-server 2.1.1 の `serve` は `port: 0` で空いているポートを選ぶので、テストで起動して `fetch` し `close` できた。Hono・@hono/node-server・zod は型定義を同梱する |
