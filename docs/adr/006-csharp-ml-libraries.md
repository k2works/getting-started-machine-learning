...
| 第 11〜12 章（B32） | SDCA の L2 の尺度は `2 × alpha / 件数` で自作のリッジ回帰と一致する（ADR 004 と同じ）。罰則が弱いと反復が収束しきらないので、突き合わせには十分強い罰則が要る（30 件で alpha=1 は 1e-3 に収まらず、alpha=10 なら収まる）。特徴量が標準化済みなら `NormalizeMeanVariance` を入れずに済み、自作との差が第 7 章より小さくなる。`Shuffle = false`・`NumberOfThreads = 1`・`ConvergenceTolerance = 1e-7` で再現性が取れる。評価指標・交差検証の数値は F# 版と表示の桁まで一致した |
| 第 13〜14 章（B32） | ML.NET に PCA は「ある」が部分的で、`Transforms.ProjectToPrincipalComponents`（ランダム化 PCA）は主成分の座標だけを返し、主成分・固有値・寄与率を取り出す API が無い。寄与率が要るので自作（ヤコビ法）を最終実装とし、ML.NET は座標の突き合わせ相手にする。`ensureZeroMean` の既定は false で、true にして退化したデータに使うと NaN を返す。`KMeansTrainer.Options` に初期中心を渡すフィールドは無い（`InitializationAlgorithm` だけ）ので、F# 版が FSharp.Stats で行った「同じ初期中心での完全一致」は C# ではできず、SSE の比較にとどまる。クラスタ番号は 1 始まり。C# の文法では `(Point)[.. values]` が添字アクセスと読まれて CS0119 になり、xUnit の `Assert.Equal(expected, actual, digits)` は桁が 0〜15 の外だと xUnit2016 でビルドが止まる |
